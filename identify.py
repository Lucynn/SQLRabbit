...
def identifyMySQLBoolean(url, params, payloads, trueFalsePayloads, d, cookies=None):
    ...
    try:
        ...
        for k, v in params.items():
            ...
            rTrue = sendReq(url, data=newParamsTrue, headers=headers, cookies=cookies) if d else sendReq(url, params=newParamsTrue, headers=headers, cookies=cookies)
            rFalse = sendReq(url, data=newParamsFalse, headers=headers, cookies=cookies) if d else sendReq(url, params=newParamsFalse, headers=headers, cookies=cookies)
            for payload in payloads:
                params[k] = v + payload
                try:
                    # Send the request with the payload
                    r2 = sendReq(url, data=params, headers=headers, cookies=cookies) if d else sendReq(url, params=params, headers=headers, cookies=cookies)
